<template>
  ...
  <div>
    <div class="flex justify-between items-center flex-wrap">
      <Title></Title>
      <div class="mr-4 my-2 bg-slate-400 p-4 rounded-md shadow-md flex flex-wrap items-center justify-between space-x-2 w-8/12"> 
        
        <SelectionList :election="dataSrc.election"></SelectionList>

        <div>
          <label for="selectElectionCode">Election: </label>
          <select name="selectElectionCode" class="cursor-pointer" v-model="selectedElectionData">
        ...
        <div>
          ...
          <select name="selectMapShading" class="cursor-pointer" v-model="selectedMapShading" value="winner">
        ...
        <div>
          ...
          <select name="selectMapFilter" class="cursor-pointer" v-model="selectedMapFilter" value="all">
            <option v-for="name of Object.keys(mapFilters)" :key="mapFilters[name]" :value="mapFilters[name]">{{name}}</option>
          </select> 
        </div>

        <div class=" basis-full place-self-start">
          <input id="selectConstituency" ref="selectConst" v-model="constituency" placeholder="Select Constituency" list="selectionList"> 
          <button id="selectConstituencyButton" @click="changeFocus($refs['selectConst'].value)">Select</button>
        </div>
